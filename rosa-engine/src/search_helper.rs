...
use crate::{eval, make, mv::mv_gen, search::*, thread_search::*};
use rosa_lib::pos;

use std::sync::mpsc;

/// Iterative deepening
pub fn search_helper(mut p: pos::Pos, sender: mpsc::Sender<ThreadReport>, stop: Stop) {
    let mut depth = 0;
    ...
    loop {
        ...
        let mut best_mv;
        let mut ponder = None;
        let mut data = Data::new(depth, stop);
